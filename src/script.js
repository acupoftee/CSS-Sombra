...
audio.play();
const codeString = `Editing Gabriel Reyes's Wikipedia page......
....
....
....
#include <stdio.h>
...
`;
// for each character, append it to the code div.
// if we reach the maximum amount of lines, scroll by
// 1 pixel

let codeContent = " ";

const destination = document.getElementById("code");
const maxLines = 16;

for (let i = 0; i < codeString.length; i++) {
  setTimeout(() => {
    codeContent += codeString[i];
    destination.innerHTML = codeContent;
    if (i > maxLines) {
      destination.scrollBy(0, 1);
    }
  }, 50 * i);
}
